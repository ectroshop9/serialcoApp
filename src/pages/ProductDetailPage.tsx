...
export default function ProductDetailPage() {
  ...
  const handleDownload = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      closeSerialModal();
      alert(`تم خصم ${product?.token_cost || 500} توكن - جاري التحميل: ${product?.name}`);
    }, 1000);
  };
  ...
  const handleOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && (data.success || data.id)) {
        ...
        setOrderError(data.message || 'فشل في إرسال الطلب');
      }
    } catch {
  ...
  if (fetchError || !product) {
    return (
      <div className="card" style={{ textAlign: 'center', padding: '60px 20px', maxWidth: 480, margin: '40px auto' }}>
        ...
        <button type="button" onClick={() => navigate('/store')} className="btn btn-primary">العودة للمتجر</button>
      </div>
    );
  }
  ...
  if (submitted) {
    return (
      <div className="card anim-scale-in" style={{ maxWidth: 500, margin: '40px auto', padding: 40, textAlign: 'center' }}>
        ...
        <button type="button" onClick={() => navigate('/store')} className="btn btn-primary btn-block">العودة للمتجر</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div className="card" style={{ padding: 24 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 32, alignItems: 'start' }}>
          ...
          <div style={{ background: 'var(--bg-subtle, #f8fafc)', borderRadius: 16, overflow: 'hidden', textAlign: 'center' }}>
            <img src={product.image || '/placeholder.jpg'} alt={product.name} style={{ width: '100%', maxHeight: 400, objectFit: 'cover', display: 'block' }} />
          </div>

          <div>
            <h1 style={{ fontSize: 24, fontWeight: 900, marginBottom: 12, lineHeight: 1.3 }}>{product.name}</h1>
            {product.description && (
              <p style={{ color: 'var(--text-muted)', marginBottom: 20, lineHeight: 1.7, fontSize: 14 }}>{product.description}</p>
            )}
            ...
            {product.product_type === 'digital' && product.token_cost && (
              <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--accent)', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {product.product_type === 'digital' ? (
              <button type="button" onClick={() => setSerialModal(true)} className="btn btn-primary btn-block btn-lg">
                <Coins style={{ width: 20, height: 20 }} /> استخدام التوكن
              </button>
            ) : (
              <form onSubmit={handleOrder} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div>
                  ...
                  <input className="field-input" placeholder="مثال: محمد أحمد" value={form.full_name} onChange={e => setForm({ ...form, full_name: e.target.value })} required disabled={submitting} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 13, fontWeight: 700, marginBottom: 6 }}>رقم الهاتف *</label>
                  <input type="tel" className="field-input" placeholder="05xxxxxxxx" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} required disabled={submitting} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 13, fontWeight: 700, marginBottom: 6 }}>العنوان التفصيلي *</label>
                  <input className="field-input" placeholder="المدينة، الحي، اسم الشارع" value={form.address} onChange={e => setForm({ ...form, address: e.target.value })} required disabled={submitting} />
                </div>
                <div className="field">
                  <label style={{ display: 'block', fontSize: 13, fontWeight: 700, marginBottom: 6 }}>الكمية</label>
                  <input className="field-input" type="number" min="1" value={form.quantity} onChange={e => setForm({ ...form, quantity: Math.max(1, parseInt(e.target.value) || 1) })} disabled={submitting} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 13, fontWeight: 700, marginBottom: 6 }}>ملاحظات إضافية</label>
                  <textarea className="field-input" placeholder="ملاحظات حول وقت التسليم..." value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} rows={3} disabled={submitting} />
                </div>
                <button type="submit" className="btn btn-primary btn-block btn-lg" disabled={submitting}>
                  {submitting ? <><Loader2 style={{ width: 20, height: 20 }} className="spin" /> جاري إرسال الطلب...</> : <><ShoppingCart style={{ width: 20, height: 20 }} /> تأكيد الطلب (الدفع عند الاستلام)</>}
                </button>
              </form>
            )}
          </div>
      ...
      <Modal isOpen={serialModal} onClose={closeSerialModal} title="استخدام التوكن">
        <form onSubmit={handleDownload} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            ...
            <input className="field-input" placeholder="أدخل رقم السيريال الخاص بك" value={serial} onChange={e => setSerial(e.target.value)} required disabled={downloading} />
          </div>
          <div>
          ...
          <button type="submit" className="btn btn-primary btn-block" disabled={downloading}>
            {downloading ? <><Loader2 style={{ width: 18, height: 18 }} className="spin" /> جاري الخصم...</> : <><Coins style={{ width: 18, height: 18 }} /> خصم التوكن والتحميل</>}
          </button>
        </form>
      </Modal>
    </div>

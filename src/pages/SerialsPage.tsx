...
export default function SerialsPage() {
  ...
  // حساب النسبة المئوية بأمان وتجنب مشكلة القسمة على صفر (NaN)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div className="anim-fade-up">
        ...
        <p className="page-desc">إدارة السيريالات الخاصة بك ومتابعة الرصيد المتاح</p>
      </div>
        ...
        <div className="card" style={{ padding: 24 }}>
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 24, fontWeight: 900, color: 'var(--primary)', fontFamily: 'monospace' }}>
                {user.serialCode}
              </span>
              <button
                type="button"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ background: 'var(--bg-primary)', borderRadius: 12, padding: 16, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 900, color: '#10b981' }}>{remaining}</div>
            </div>
            <div style={{ background: 'var(--bg-primary)', borderRadius: 12, padding: 16, textAlign: 'center' }}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>المستخدم</div>
              <div style={{ fontSize: 28, fontWeight: 900, color: '#6366f1' }}>{used}</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="card" style={{ padding: 24 }}>
        <div className="row" style={{ marginBottom: 16 }}>
          <div className="icon-box icon-box-md" style={{ background: 'var(--grad-success)' }}>
        ...
        <form onSubmit={check} className="row" style={{ gap: 12, marginBottom: 16 }}>
          <input
            ...
            onChange={e => {
              ...
              if (checkResult) setCheckResult(null); // إعادة تعيين النتيجة عند تعديل النص
            }}
        ...
        {checkResult && (
          ...
          >
            ...
            <div style={{ fontSize: 28, fontWeight: 900, color: '#10b981' }}>
              {checkResult.balance} تحميل متبقي
            </div>
          </div>
        )}
      </div>

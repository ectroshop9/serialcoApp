...
import { Key, Plus, Search, CheckCircle, Copy } from 'lucide-react';
...
export default function SerialsPage() {
  ...
  const check = (e: React.FormEvent) => {
    e.preventDefault();
    setCheckResult({ balance: 47 });
  };

  const copy = () => {
    if (user?.serialCode) navigator.clipboard.writeText(user.serialCode);
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div className="anim-fade-up">
        ...
        <p className="page-desc">إدارة السيريالات الخاصة بك</p>
      </div>

      {msg && (
        <div className="anim-scale-in" style={{ background: '#10b981', color: '#fff', padding: '12px 20px', borderRadius: 12, fontWeight: 700, textAlign: 'center' }}>{msg}</div>
      )}
        ...
        <div className="card" style={{ padding: 24 }}>
          ...
          <div style={{ background: 'var(--bg-primary)', borderRadius: 14, padding: 20, textAlign: 'center', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8 }}>
              ...
              <button onClick={copy} className="btn btn-ghost btn-sm"><Copy style={{ width: 16 }} /></button>
            </div>
            <div style={{ marginTop: 12, height: 6, borderRadius: 3, background: 'var(--border)', overflow: 'hidden' }}>
              <div style={{ width: `${(user.remainingDownloads / (user.remainingDownloads + user.totalDownloads)) * 100}%`, height: '100%', background: 'var(--grad-primary)' }} />
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ background: 'var(--bg-primary)', borderRadius: 12, padding: 16, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 900, color: '#10b981' }}>{user.remainingDownloads}</div>
            </div>
            <div style={{ background: 'var(--bg-primary)', borderRadius: 12, padding: 16, textAlign: 'center' }}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>المستخدم</div>
              <div style={{ fontSize: 28, fontWeight: 900, color: '#6366f1' }}>{user.totalDownloads}</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="card" style={{ padding: 24 }}>
        ...
        <form onSubmit={addSerial} className="row" style={{ gap: 12 }}>
          <input className="field-input" style={{ flex: 1 }} value={newSerial} onChange={e => setNewSerial(e.target.value)} placeholder="أدخل كود السيريال" required />
      ...
      <div className="card" style={{ padding: 24 }}>
        <div className="row" style={{ marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 800 }}>فحص رصيد سيريال</div>
        </div>
        <form onSubmit={check} className="row" style={{ gap: 12, marginBottom: 16 }}>
          <input className="field-input" style={{ flex: 1 }} value={checkSerial} onChange={e => setCheckSerial(e.target.value)} placeholder="أدخل كود السيريال للفحص" required />
        ...
        {checkResult && (
          <div className="anim-scale-in" style={{ textAlign: 'center', padding: 20, borderRadius: 14, background: 'rgba(16,185,129,0.08)', border: '1.5px solid rgba(16,185,129,0.2)' }}>
            ...
            <div style={{ fontSize: 28, fontWeight: 900, color: '#10b981' }}>{checkResult.balance} تحميل متبقي</div>
          </div>
        )}
      </div>
    </div>
